...
/*This fxn send at command and get the reply*/
char GSM_MQTT::_sendAT(char *command, unsigned long waitms)
{
  ...
  //  GsmSerial.println(PrevMillis);
  //  GsmSerial.println(currentMillis);
  ...
  while (GsmSerial.available())
  {
    ...
    if (MQTT.TCP_Flag == false)
    {
      ...
      if (inChar == '\n')
      {
        ...
        stringComplete = true;
        GsmSerial.print(MQTT.inputString);
    ...
    else
    {
      ...
      if ((ReceivedMessageType >= CONNECT) && (ReceivedMessageType <= DISCONNECT))
      {
        ...
        while ( (NextLengthByte == true) && (MQTT.TCP_Flag == true))
        {
          if (Serial.available())
          {
            ...
            if ((((Cchar & 0xFF) == 'C') && ((inChar & 0xFF) == 'L') && (MQTT.length == 0)) || (((Cchar & 0xFF) == '+') && ((inChar & 0xFF) == 'P') && (MQTT.length == 0)))
            {
              ...
              MQTT.MQTT_Flag = false;
              MQTT.pingFlag = false;
              GsmSerial.println("Disconnecting");
            }
            else
            {
              if ((inChar & 128) == 128)
        ...
        if (MQTT.TCP_Flag == true)
        {
          ...
          {
            MQTT.inputString[uint32_t(MQTT.index++)] = (char)GsmSerial.read();
          ...
          else if (ReceivedMessageType == PUBLISH)
          {
            ...
            {
              GsmSerial.print(MQTT.inputString[iter]);
          ...
          {
            GsmSerial.print("Message ID :");
            MQTT.publishREL(0, MQTT.inputString[0] * 256 + MQTT.inputString[1]) ;
            GsmSerial.println(MQTT.inputString[0] * 256 + MQTT.inputString[1]) ;
      ...
      {
        GsmSerial.print("Received :Unknown Message Type :");
        GsmSerial.println(inChar);
      }
    }
  }
}
